...
export const WebUiGemini = ({ apiConfiguration, setApiConfigurationField }: WebUiGeminiProps) => {
	...
	return (
		<div className="flex flex-col space-y-4">
			...
			<div>
				<VSCodeTextArea
					value={apiConfiguration.webUiGeminiRegenerationPrompt || ""}
					className="w-full"
					onInput={handleInputChange("webUiGeminiRegenerationPrompt")}
					rows={2}>
					<label className="block font-medium mb-1">
						{t("settings:providers.webUiGemini.regenerationPromptLabel")}
					</label>
				</VSCodeTextArea>
				<p className="text-xs text-vscode-descriptionForeground mt-1">
					{t("settings:providers.webUiGemini.regenerationPromptDescription")}
				</p>
			</div>

			<div>
				<VSCodeTextArea
					value={apiConfiguration.webUiGeminiMalformedTokenList || ""}
					className="w-full"
					onInput={handleInputChange("webUiGeminiMalformedTokenList")}
					rows={2}>
					<label className="block font-medium mb-1">
						{t("settings:providers.webUiGemini.malformedTokenListLabel")}
					</label>
				</VSCodeTextArea>
				<p className="text-xs text-vscode-descriptionForeground mt-1">
					{t("settings:providers.webUiGemini.malformedTokenListDescription")}
				</p>
			</div>
